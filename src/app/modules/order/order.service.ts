...
const createOrder = async (
  ...
  const newOrder = await prisma.$transaction(async (transaction: Prisma.TransactionClient) => {
...
const updateOrder = async (
  ...
  const updatedOrder = await prisma.$transaction(async (transaction: Prisma.TransactionClient) => {
    ...
    if (order.status === OrderStatus.CANCELED) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'This order has already been canceled');
    }
